...
<head>
	<title>Shop quần áo Nam 2024</title>
		...
		$(document).ready(function() {
			...
			$(".guidonhang").click(function() {
				...
				if (errors.length == 0) {
					...
					var quan_huyen = ''; // Make sure to set this variable appropriately if needed
					var sonha_tenduong = $("input[name='sonha-tenduong']").val();
					var phuong_xa = $("input[name='phuong-xa']").val();

					// Using fetch to send the data
					fetch("xuli/gui-don-hang.php?" + new URLSearchParams({
							name: name,
							email: email,
							sdt: sdt,
							tinh: tinh_thanh,
						...
						.then(response => {
							if (response.ok) {
								...
								console.error("API not found. Status:", response.status);
							}
						})
						.then(data => {
